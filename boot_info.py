...
def file_operations(args):
    if args.type == 'mbr':
        ...
        for i in range(len(MBR.status)):
            offset = int(0x1BE) + (i * 16)
            entry = mbr[offset:offset + 16]
            partition_start = int.from_bytes(entry[8:12], byteorder="little")
            rawfile.seek(partition_start)
            boot_record = rawfile.read(512)
            partitionNum = i + 1
            print(f"Partition number: {str(partitionNum)}")
            last = ' '.join([f'{b:02X}' for b in boot_record[-16:]])
            print(f"Last 16 bytes of boot record: {last}")
